...
char * regNames[8] = {"%eax", "%ecx", "%edx", "%ebx", "%esp", "%ebp", "%esi","%edi"};
/*char * getRegister(int a){
	char* reg = malloc;	
	switch(a){
		case 0:
			reg = "%eax";
...
void rrmovl() {
	...
	if ((tempByte -> low >= 0 && tempByte -> low <= 7)&& (tempByte -> high >= 0 && tempByte -> high <= 7)) { //check if byte 1 contains two valid register number
		//printf("Instruction Address: Ox%X\nInstructions: %x%x%x%x\n" , eip-1, (tempByte-1)->low, (tempByte-1)->high, tempByte->low, tempByte->high);
		printf("rrmovl %s, %s\n\n", regNames[tempByte->low], regNames[tempByte->high]);
...
void rmmovl() {
	...
	if ((tempByte -> low >= 0 && tempByte -> low <= 7)&& (tempByte -> high >= 0 && tempByte -> high <= 7)) { //check if byte 1 contains two valid register number
		printf("rmmovl %s, $Ox%X(%s)\n\n", eip-1, regNames[tempByte->low], *num, regNames[tempByte->high]);
...
void mrmovl() {
	...
	if ((tempByte -> low >= 0 && tempByte -> low <= 7)&& (tempByte -> high >= 0 && tempByte -> high <= 7)) { //check if byte 1 contains two valid register numbers
		printf("mrmovl %Ox%X(%s), %s\n\n", *num, regNames[tempByte->low], regNames[tempByte->high]);				
